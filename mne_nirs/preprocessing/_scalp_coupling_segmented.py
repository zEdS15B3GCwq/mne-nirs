    ...
    for window in range(n_windows):
        ...
        if n_wavelengths == 2:
            ...
            for ii in range(0, len(picks), 2):
                ...
                with np.errstate(invalid="ignore"):  # in mne-python.mne.preprocessing.nirs.scalp_coupling_index
                    c = np.corrcoef(c1, c2)[0][1]
                if not np.isfinite(c):  # in mne-python.mne.preprocessing.nirs.scalp_coupling_index
                    c = 0
                scores[ii, window] = c
